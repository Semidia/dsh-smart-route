...
export function apply(ctx, raw) {
    ...
    // 注册 settings namespace（与官方插件一致：在 settings 服务 mount 回调里同步注册）。
    // 注意：不能用 installSettingsSection（它走 ctx.inject 异步子插件，client 的
    // /api/settings.describe 可能看不到）；直接 register 到调用方 fiber。
