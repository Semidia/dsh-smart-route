...
import { LlmAdapter, LlmError, CallId, EMPTY_RESPONSE_CODE, RetryPolicySchema, resolveRetryPolicy } from '@deepseek-ai/dsh-llm';
...
export const Config = z.object({
    ...
    // 虚拟 provider 的整体请求重试策略（dsh-llm-retry 使用）。
    // 缺省时走 dsh-llm 默认（normal + maxRetries=2）。
...
class SmartRouteAdapter extends LlmAdapter {
    ...
    providerInfo(provider) {
        return { id: provider, name: '智能路由链' };
    }
    providerRetryPolicy(_provider) {
        // 返回 settings 段配置的全局 retryPolicy；undefined 时由 dsh-llm
        // prepareRoutes 回落为默认策略（normal + maxRetries=2）。
        return this.#options.retryPolicy();
    }
    listModels(provider) {
        const state = this.#options.state();
        const chainNames = Object.keys(state.chains || {});
        if (chainNames.length === 0) {
            return Promise.resolve([{ provider, id: 'auto', name: '自动路由', inputModalities: ['text'] }]);
        }
        return Promise.resolve(chainNames.map((name) => ({
            provider,
            id: name,
            name: name + (name === state.defaultChain ? '（默认）' : ''),
            inputModalities: ['text'],
        })));
    }
    async resolveModel(provider, model, _signal) {
        const state = this.#options.state();
        const chainName = model && model !== 'auto' && state.chains && model in state.chains ? model : state.defaultChain;
        const chain = this.#options.chain(chainName);
        const first = chain.entries[0];
        if (first === undefined) {
            throw new LlmError('dsh-smart-route: 链上没有配置任何渠道', 'NO_PROVIDER');
        }
        try {
            const resolved = await this.#options.resolveModelInfo(first.provider, first.model);
            if (resolved !== undefined) {
                // llm 服务校验 resolved.provider === provider 且 resolved.id === model，
                // 因此保留首渠道的元数据（context / maxTokens 等），但 provider/id/name 改回虚拟路由的值。
                return { ...resolved, provider, id: model, name: model };
            }
        } catch {
    ...
    async *stream(options) {
        ...
        if (modelName && modelName !== 'auto' && state.chains && modelName in state.chains) {
            await this.#options.updateDefaultChain(modelName);
        }
        const chain = this.#options.chain(modelName);
...
export function apply(ctx, raw) {
    ...
    // ── settings 读写辅助（用于 updateDefaultChain）────────────────
    ...
    const updateDefaultChain = async (name) => {
        const base = readSection() ?? {};
        if (base.defaultChain === name) return;
        await writeSection({ ...base, defaultChain: name });
    };

    const router = new SmartRouteAdapter({
        state: () => state(),
        chain: (name) => chainEntries(name || current().defaultChain),
        updateDefaultChain,
    ...
    let registration = ctx.llm.registerAdapter([VIRTUAL_PROVIDER], router);
    ...
    // retryPolicy 在 prepareRoutes 时被捕获进 registration；settings 段
    // retryPolicy 变化时重跑 prepareRoutes 让新策略热生效（与 pi-ai 一致）。
    let lastRetryPolicyRaw = JSON.stringify(settingsScope.get()?.retryPolicy ?? null);
    settingsScope.watch(() => {
        const raw = current();
        const fingerprint = JSON.stringify(raw?.retryPolicy ?? null);
        state();
        if (fingerprint !== lastRetryPolicyRaw) {
            lastRetryPolicyRaw = fingerprint;
            registration.replace([VIRTUAL_PROVIDER]);
        }
    });
...
function resolveState(raw) {
    ...
    return {
        ...
        chains: raw.chains,
        retryPolicy: raw.retryPolicy === undefined ? undefined : resolveRetryPolicy(raw.retryPolicy, 'smart-route: retryPolicy'),
    };
}
